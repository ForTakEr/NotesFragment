...
namespace NotesFragment
{
    public class PlayNoteFragment : Fragment
    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            StaticPlayId = PlayId;
            List<int> noteIdList = DatabaseService.NotesList.Select(x => x.Id).ToList();
            List<string> titlesList = DatabaseService.NotesList.Select(x => x.Title).ToList();
            List<string> notesList = DatabaseService.NotesList.Select(x => x.Note).ToList();
